...
const LaporanScreen = () => {
    ...
    const handleVote = async (laporanId: number) => {
        try {
            ...
            setLaporans(prev =>
                prev.map(l => {
                    if (l.laporan_id === laporanId) {
                    const alreadyVoted = l.votes?.some(v => v.userId === user.user_id);
                    if (alreadyVoted) return l;
                    const updatedVotes: Vote[] = [...(l.votes || []), { userId: user.user_id }];
                    return { ...l, vote: l.vote + 1, votes: updatedVotes };
                    }
                    return l;
                }),
            );
                    ...
                    {laporans.length > 0 ? (
                        laporans.map(laporan => (
                            <ReportCard
                                ...
                                buttonDisabled={laporan.votes?.some(v => v.userId === user?.user_id)}
                                buttonLabel="Vote"
